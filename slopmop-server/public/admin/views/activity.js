...
export function activityCharts(d) {
  ...
  const byHour = barChart(d.hourOfDay, [{ label: "Checks", color: "var(--blue)", get: (h) => h.checks, legend: false }], {
    x: (h) => pad2(h.hour),
    tipTitle: (h) => `${pad2(h.hour)}:00–${pad2(h.hour)}:59 UTC`,
    tickEvery: 3,
  });
  ...
  return [
    ...
    el("div", { class: "grid two" }, card("Installs", installs, "Devices that checked something, and devices first seen"), card("When people use it (UTC hour of day)", byHour)),
  ];
}
