...
function limitsEditor(r, defaults) {
  ...
  const alias = el("input", { type: "text", class: "field field-device", maxlength: "40", placeholder: "e.g. My laptop", value: r.alias ?? "", "aria-label": `Name for ${r.device}` });
  return el(
    "div",
    { class: "tools" },
    el("label", null, "Name ", alias),
    ...
    el("button", { class: "primary", onclick: () => {
      ...
      void setLimits(r.device, { dailyLimit: d, hourlyLimit: h, alias: alias.value.trim() || null });
    } }, "Save"),
    el("button", { onclick: () => void setLimits(r.device, { dailyLimit: null, hourlyLimit: null }) }, "Use the default limits"),
...
function row(r, defaults) {
  ...
  return [
    el("tr", null,
      ...
      el("td", { class: "acts" }, el("button", { "aria-expanded": String(V.open === r.device), onclick: () => { V.open = V.open === r.device ? null : r.device; paint(); } }, "Edit"), " ", el("label", { class: "inline" }, disableCheckbox(r), " Off")),
    ),
...
export function devicesPage() {
  ...
  const search = el("input", {
    type: "search", class: "field field-device", placeholder: "Find a device (id or name)", value: V.q, "aria-label": "Find a device by id or name", spellcheck: "false", autocomplete: "off", maxlength: "64",
