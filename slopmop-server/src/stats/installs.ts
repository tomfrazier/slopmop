...
export async function installStats(s: Scope) {
  const { now } = s;
  const { dailyLimit } = s.store.config;
  ...
  const [cap] = await s.q(`SELECT COUNT(*) n, SUM(checks >= ?) at_cap, SUM(checks >= ?) near_cap, MAX(checks) top FROM usage WHERE day = ?`, [dailyLimit, Math.ceil(dailyLimit * NEAR_CAP_FRACTION), dayKey(now)]);
  return {
...
export async function deviceTable(s: Scope) {
  const rows = await s.q(
    `SELECT e.install_hash h, SUM(e.kind IN ('scored','cached')) checks, SUM(e.kind='scored') scored, SUM(e.kind='limited') limited, SUM(e.kind='error') errors,
            ...
            i.first_seen first_seen, COALESCE(i.disabled, 0) disabled, (SELECT COUNT(*) FROM votes v WHERE v.install_hash = e.install_hash) votes
