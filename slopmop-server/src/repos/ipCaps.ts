...
export class IpCapRepo {
  ...
  /** Same atomic-upsert shape as CapRepo.consume: the increment only happens while under the limit, so concurrent requests can't overshoot it. */
  async consume(ipHash: string, limit: number): Promise<{ ok: boolean; count: number }> {
    const t = this.d.now();
    const hour = hourKey(t);
    const r = await this.d.db.execute(
      `INSERT INTO ip_usage (ip_hash, hour, checks) VALUES (?, ?, 1)
       ON CONFLICT(ip_hash, hour) DO UPDATE SET checks = checks + 1 WHERE checks < ?
       RETURNING checks`,
      [ipHash, hour, limit],
    );
