...
export async function clientsRoute(request: Request, ctx: Ctx): Promise<Response> {
  ...
  const setsAlias = body.alias !== undefined;
  if (setsAlias && body.alias !== null && (typeof body.alias !== "string" || body.alias.trim().length > MAX_ALIAS_CHARS)) throw new HttpError(422, "invalid_input", `An alias is text of up to ${MAX_ALIAS_CHARS} characters, or null to remove it.`);
  if (typeof body.device !== "string" || (typeof body.disabled !== "boolean" && !setsLimits && !setsAlias)) throw new HttpError(422, "invalid_input", "Send {device, disabled?: true|false, dailyLimit?: number|null, hourlyLimit?: number|null, alias?: string|null}.");
  ...
  if (setsAlias) await clients.setAlias(found.hash, body.alias === null || !(body.alias as string).trim() ? null : (body.alias as string).trim());
  ...
  return json(200, { device: deviceId(found.hash), ...(typeof body.disabled === "boolean" ? { disabled: body.disabled } : {}), ...(setsAlias ? { alias: body.alias === null || !(body.alias as string).trim() ? null : (body.alias as string).trim() } : {}), ...(setsLimits ? { dailyLimit: daily ?? null, hourlyLimit: hourly ?? null } : {}) });
}
